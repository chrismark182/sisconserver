    ...
    <div class="row">
        <div class="input-field col s12 m6 l4">
            <select id="tipo_doc" required>
                ...
                <?php                    
                    foreach ($tipo_documentos as $row): 
                        $selected = ($movdoc->TIDORE_N_ID == $row->TIDORE_N_ID) ? 'selected' : '';
                ?>
                    <option value="<?= $row->TIDORE_N_ID?>" <?= $selected ?>> <?= $row->TIDORE_C_ABREVIATURA ?> </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="input-field col s12 m6 l12">
            <textarea id="observaciones" class="materialize-textarea"><?= $movdoc->MOVDOC_C_OBSERVACIONES?></textarea>
...
<script>
    ...
    function verificar ()
    {
        if(parseInt(document.getElementById('tipo_doc').value) != 0 && document.getElementById('nro_documento').value != ''){
